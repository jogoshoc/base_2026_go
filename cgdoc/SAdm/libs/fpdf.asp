...
/*
		if(!lib.isset(this.images[xfile]))
			...
			if(xtype=="jpg" || xtype=="jpeg")
				xinfo=this._parsejpg(xfile);
			else 
				return;
...
*/		
	...
	this._putpages=function _putpages()
		...
		for(xn=1;xn<=xnb;xn++)
		 ...
		 this.offsets[1]=lib.strlen(this.buffer)+this.offset;
		 this._out("1 0 obj");
	...
	this._putimages=function _putimages()
		...
		for(xfile  in this.images)
			...
			if(xinfo["cs"]=="Indexed")
				{
				this._out("/ColorSpace [/Indexed /DeviceRGB " + (vblenb(xinfo["pal"])/3-1) + " " + (this.n+1) + " 0 R]");
				}
			else
				{
			...
			if(lib.isset(xinfo["trns"]) && lib.is_array(xinfo["trns"]))
			 ...
			 this._out("stream");
			 response.write(this.buffer);
			 response.binarywrite(xinfo["data"]);
			 this.offset+=lib.strlen(this.buffer)+vblenb(xinfo["data"]);
			 this.buffer="";
			 this._out("endstream");
//			 this._putstream(xinfo["data"]);
			...
			if(xinfo["cs"]=="Indexed")
				...
				xinfo["pal"];
				 this._out("<<" + xfilter + "/Length " + vblenb(xpal) + ">>");
	
			 this._out("stream");
			 response.write(this.buffer);
			 response.binarywrite(xpal);
			 this.offset+=lib.strlen(this.buffer)+vblenb(xpal);
			 this.buffer="";
			 this._out("endstream");
//				 this._putstream(xpal);
	...
	this._putresources=function _putresources()
		...
		this._putfonts();
		this._putimages();
		this.offsets[2]=lib.strlen(this.buffer)+this.offset;
		this._out("2 0 obj");
		this._out("<<");
	...
	this._enddoc=function _enddoc()
		{
		 ...
		 this._out(">>");
		 this._out("endobj");
		xo=lib.strlen(this.buffer)+this.offset;
		 this._out("xref");
	...
	this._newobj=function _newobj()
		{
		 this.n++;
		 this.offsets[this.n]=lib.strlen(this.buffer)+this.offset;
