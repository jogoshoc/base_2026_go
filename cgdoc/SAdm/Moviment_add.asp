...
'//	insert masterkey value if exists and if not specified
	if Session(strTableName & "_mastertable")="Cadastro" then
...
if getRequestForm("copyid1")<>"" then
	...
	if not rs.EOF then
		...
		defvalues("OrigNome")=dbvalue(rs("OrigNome"))
...
'//call CopyOnLoad event
	...
	defvalues("OrigNome")=Session("OwnerID")
...
if readavalues then
	...
	if avalues.exists("OrigNome") then
		defvalues("OrigNome")= avalues("OrigNome")
	end if
...
smarty_display("Moviment_add.htm")
