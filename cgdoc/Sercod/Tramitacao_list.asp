...
if useAJAX then
	...
	includes = includes & "var MASTER_PREVIEW_TABLE = ""Tramitacao_masterpreview.asp"";" & vbCrLf
end if
...
'	 regular search
	...
	if SESSION(strTableName & "_searchfield")<> "" then
		...
		where=StrWhere("DtMovim", strSearchFor, strSearchOption, "")
		if where<>"" then sstrWhere=sstrWhere &  " or " & where
		where=StrWhere("OrigNome", strSearchFor, strSearchOption, "")
		if where<>"" then sstrWhere=sstrWhere &  " or " & where
		where=StrWhere("DestNome", strSearchFor, strSearchOption, "")
		if where<>"" then sstrWhere=sstrWhere &  " or " & where
		where=StrWhere("Obs", strSearchFor, strSearchOption, "")
		if where<>"" then sstrWhere=sstrWhere &  " or " & where
		where=StrWhere("Prazo", strSearchFor, strSearchOption, "")
		if where<>"" then sstrWhere=sstrWhere &  " or " & where
...
'	write pagination
smarty.Add "pagination","<script language=""JavaScript"">WritePagination(" & mypage & "," & maxpages & ");function GotoPage(nPageNumber){window.location='Tramitacao_list.asp?goto='+nPageNumber;}</script>"
...
'	key fields
			...
			recno=recno+1
'	detail tables
			masterquery="mastertable=Tramitacao"
			masterquery=masterquery & "&masterkey1=" & SafeURLEncode(dbvalue(rs("NrProtoc")))
			row.Add col & "Moviment2_masterkeys",masterquery
			masterquery="mastertable=Tramitacao"
			masterquery=masterquery & "&masterkey1=" & SafeURLEncode(dbvalue(rs("NrProtoc")))
			row.Add col & "moviment_sec_masterkeys",masterquery
'	edit page link
			editlink=""
...
'	//	field selection
		...
		if SESSION(strTableName& "_searchfield")="DtMovim" then smarty.Add "search_DtMovim","selected"
		if SESSION(strTableName& "_searchfield")="OrigNome" then smarty.Add "search_OrigNome","selected"
		if SESSION(strTableName& "_searchfield")="DestNome" then smarty.Add "search_DestNome","selected"
		if SESSION(strTableName& "_searchfield")="Obs" then smarty.Add "search_Obs","selected"
		if SESSION(strTableName& "_searchfield")="Prazo" then smarty.Add "search_Prazo","selected"
...
strPerm = GetUserPermissions("Moviment2")
f=false
if instr(strPerm, "A")<>0 or instr(strPerm, "S")<>0 then f=true
smarty.Add "allow_Moviment2",f
strPerm = GetUserPermissions("moviment_sec")
f=false
if instr(strPerm, "A")<>0 or instr(strPerm, "S")<>0 then f=true
smarty.Add "allow_moviment_sec",f
...
smarty_display("Tramitacao_list.htm")
