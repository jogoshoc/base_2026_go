...
<!--#include file="include/Moviment_variables.asp"-->
...
smarty.Add "mastertable",mastertable
smarty.Add "mastertable_short",GetTableURL(mastertable)
...
if useAJAX then
	includes = includes & "var SUGGEST_TABLE = ""Moviment_searchsuggest.asp"";" & vbCrLf
	includes = includes & "var MASTER_PREVIEW_TABLE = ""Moviment_masterpreview.asp"";" & vbCrLf
end if
...
smarty.Add "order_dir_DtMovim","a"
smarty.Add "order_dir_OrigNome","a"
smarty.Add "order_dir_DestNome","a"
smarty.Add "order_dir_Obs","a"
smarty.Add "order_dir_Prazo","a"
smarty.Add "order_dir_PastaArquiv","a"
smarty.Add "order_dir_Cumprido","a"
...
if SESSION(strTableName & "_orderby")<> "" then
	...
	AddDict smarty,"order_dir_DtMovim","a"
	if order_field="DtMovim" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DtMovim","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DtMovim","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_OrigNome","a"
	if order_field="OrigNome" then
		if order_dir="a" then
			AddDict smarty,"order_dir_OrigNome","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_OrigNome","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_DestNome","a"
	if order_field="DestNome" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DestNome","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DestNome","<img src=""images/" & img & ".gif"" border=0>"
	end if
	...
	AddDict smarty,"order_dir_Prazo","a"
	if order_field="Prazo" then
		if order_dir="a" then
			AddDict smarty,"order_dir_Prazo","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_Prazo","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_PastaArquiv","a"
	if order_field="PastaArquiv" then
		if order_dir="a" then
			AddDict smarty,"order_dir_PastaArquiv","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_PastaArquiv","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_Cumprido","a"
	if order_field="Cumprido" then
		if order_dir="a" then
			AddDict smarty,"order_dir_Cumprido","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_Cumprido","<img src=""images/" & img & ".gif"" border=0>"
	end if
...
if request("mdelete[]").Count>0 then
	...
	for ind=1 to request("mdelete[]").Count
		AddDict keys,"CodMov",request("mdelete1[]").Item(request("mdelete[]").Item(ind))
		AddDict keys,"NrProtoc",request("mdelete2[]").Item(request("mdelete[]").Item(ind))
...
'	write pagination
smarty.Add "pagination","<script language=""JavaScript"">WritePagination(" & mypage & "," & maxpages & ");function GotoPage(nPageNumber){window.location='Moviment_list.asp?goto='+nPageNumber;}</script>"
...
'	key fields
			row.Add col & "id1",my_htmlspecialchars(dbvalue(rs("CodMov")))
			row.Add col & "id2",my_htmlspecialchars(dbvalue(rs("NrProtoc")))
			row.Add col & "recno",recno
			recno=recno+1
'	detail tables
'	edit page link
			editlink=""
			editlink=editlink & "editid1=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("CodMov"))))
			editlink=editlink & "&editid2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("NrProtoc"))))
			row.Add col & "editlink",editlink

			copylink=""
			copylink=copylink & "copyid1=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("CodMov"))))
			copylink=copylink & "&copyid2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("NrProtoc"))))
			row.Add col & "copylink",copylink
			keylink=""
			keylink=keylink & "&key1=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("CodMov"))))
			keylink=keylink & "&key2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("NrProtoc"))))
...
'	DtMovim - Short Date
	value=""
				value = ProcessLargeText(GetData(rs,"DtMovim", "Short Date"),"field=DtMovim" & keylink,"",MODE_LIST)
			row.Add col & "DtMovim_value",value

'	OrigNome - 
	value=""
				if len(rs("OrigNome"))>0 then
				strdata = make_db_value("OrigNome",rs("OrigNome"),"","")
				LookupSQL="SELECT "
				...
				if not rss.EOF then
					...
					value=ProcessLargeText(GetData(rs,"OrigNome", ""),"field=OrigNome" & keylink,"",MODE_LIST)
				end if
				rss.Close
			else
				value=""
			end if
			row.Add col & "OrigNome_value",value

'	DestNome - 
	value=""
				if len(rs("DestNome"))>0 then
				strdata = make_db_value("DestNome",rs("DestNome"),"","")
				LookupSQL="SELECT "
							LookupSQL=LookupSQL & "[Orgao]"
				LookupSQL=LookupSQL & " FROM [Orgaos] WHERE [Orgao] = " & strdata
							LogInfo(LookupSQL)
				...
				if not rss.EOF then
					...
					value=ProcessLargeText(GetData(rs,"DestNome", ""),"field=DestNome" & keylink,"",MODE_LIST)
				end if
				rss.Close
			else
				value=""
...
'	Prazo - Short Date
	value=""
				value = ProcessLargeText(GetData(rs,"Prazo", "Short Date"),"field=Prazo" & keylink,"",MODE_LIST)
			row.Add col & "Prazo_value",value

'	Cumprido - 
	value=""
				value = ProcessLargeText(GetData(rs,"Cumprido", ""),"field=Cumprido" & keylink,"",MODE_LIST)
			row.Add col & "Cumprido_value",value

'	PastaArquiv - 
	value=""
				value = ProcessLargeText(GetData(rs,"PastaArquiv", ""),"field=PastaArquiv" & keylink,"",MODE_LIST)
			row.Add col & "PastaArquiv_value",value
...
%>
	<!--#include file="include/Cadastro_masterlist.asp"-->
	<%
	masterkeys.Add i,SESSION(strTableName & "_masterkey1")
	i=i+1
	DisplayMasterTableInfo "Moviment", masterkeys
	AddDict smarty,"showmasterfile","Cadastro_masterlist.htm"
end if


strSQL=SESSION(strTableName & "_sql")

smarty_display("Moviment_list.htm")
