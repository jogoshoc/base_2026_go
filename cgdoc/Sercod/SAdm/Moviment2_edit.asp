...
'//	processing OrigNome - start
	value = postvalue("value_OrigNome")
	ttype=postvalue("type_OrigNome")
	toedit=true

if myRequest.Exists("value_OrigNome") or myRequest.Exists("value_OrigNome[]") or myRequest.Exists("type_OrigNome") then
			value=prepare_for_db("OrigNome",value,ttype,"")
		if vartype(value)=11 then 
			if value=false then _
				toedit=false
		end if
		if toedit then
			evalues("OrigNome") = value
			end if
	end if


'//	processibng OrigNome - end
...
'//	processing PastaArquiv - start
	value = postvalue("value_PastaArquiv")
	ttype=postvalue("type_PastaArquiv")
	toedit=true

if myRequest.Exists("value_PastaArquiv") or myRequest.Exists("value_PastaArquiv[]") or myRequest.Exists("type_PastaArquiv") then
			value=prepare_for_db("PastaArquiv",value,ttype,"")
		if vartype(value)=11 then 
			if value=false then _
				toedit=false
		end if
		if toedit then
			evalues("PastaArquiv") = value
			end if
	end if


'//	processibng PastaArquiv - end
...
rs.Open strSQL,dbConnection,1,2
if rs.EOF then
	response.redirect "Moviment2_list.asp?a=return"
	response.end
end if
...
includes=includes & "</script>"
	...
	if validatetype<>"" then bodyonload=bodyonload & "define('value_DtMovim','" & validatetype & "','Dt Movim');"
	validatetype=""
	validatetype=validatetype & "IsRequired"
	if validatetype<>"" then bodyonload=bodyonload & "define('value_OrigNome','" & validatetype & "','ORIGEM');"
	validatetype=""
	validatetype=validatetype & "IsRequired"
	if validatetype<>"" then bodyonload=bodyonload & "define('value_DestNome','" & validatetype & "','DESTINO');"
...
if useAJAX then
	includes=includes & "var AUTOCOMPLETE_TABLE='Moviment2_autocomplete.asp';" & vbcrlf
	includes=includes & "var SUGGEST_TABLE='Moviment2_searchsuggest.asp';" & vbcrlf
	includes=includes & "var SUGGEST_LOOKUP_TABLE='Moviment2_lookupsuggest.asp';" & vbcrlf
end if
...
set readonlyfields = CreateObject("Scripting.Dictionary")
	...
	if readevalues then
		smarty.Add "value_OrigNome",evalues("OrigNome")
	else
		smarty.Add "value_OrigNome",dbvalue(rs("OrigNome"))
	end if
	if readevalues then
	...
	if readevalues then
		smarty.Add "value_PastaArquiv",evalues("PastaArquiv")
	else
		smarty.Add "value_PastaArquiv",dbvalue(rs("PastaArquiv"))
	end if
	if readevalues then
...
smarty_display("Moviment2_edit.htm")
