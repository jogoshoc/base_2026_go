...
if useAJAX then
	includes = includes & "var SUGGEST_TABLE = ""_AudMoviment_searchsuggest.asp"";" & vbCrLf
	includes = includes & "var MASTER_PREVIEW_TABLE = ""_AudMoviment_masterpreview.asp"";" & vbCrLf
end if
...
smarty.Add "order_dir_CodMov","a"
smarty.Add "order_dir_NrProtoc","a"
smarty.Add "order_dir_DataAlteracao","a"
smarty.Add "order_dir_DataAnt","a"
smarty.Add "order_dir_DataMod","a"
smarty.Add "order_dir_OrigAnt","a"
smarty.Add "order_dir_OrigMod","a"
smarty.Add "order_dir_DestAnt","a"
smarty.Add "order_dir_DestMod","a"
smarty.Add "order_dir_ObsAnt","a"
smarty.Add "order_dir_ObsMod","a"
smarty.Add "order_dir_PrazoAnt","a"
smarty.Add "order_dir_PrazoMod","a"
smarty.Add "order_dir_ID","a"
...
if SESSION(strTableName & "_orderby")<> "" then
	...
	AddDict smarty,"order_dir_CodMov","a"
	if order_field="CodMov" then
		if order_dir="a" then
			AddDict smarty,"order_dir_CodMov","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_CodMov","<img src=""images/" & img & ".gif"" border=0>"
	end if
	...
	AddDict smarty,"order_dir_DataAlteracao","a"
	if order_field="DataAlteracao" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DataAlteracao","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DataAlteracao","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_DataAnt","a"
	if order_field="DataAnt" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DataAnt","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DataAnt","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_DataMod","a"
	if order_field="DataMod" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DataMod","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DataMod","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_OrigAnt","a"
	if order_field="OrigAnt" then
		if order_dir="a" then
			AddDict smarty,"order_dir_OrigAnt","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_OrigAnt","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_OrigMod","a"
	if order_field="OrigMod" then
		if order_dir="a" then
			AddDict smarty,"order_dir_OrigMod","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_OrigMod","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_DestAnt","a"
	if order_field="DestAnt" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DestAnt","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DestAnt","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_DestMod","a"
	if order_field="DestMod" then
		if order_dir="a" then
			AddDict smarty,"order_dir_DestMod","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_DestMod","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_ObsAnt","a"
	if order_field="ObsAnt" then
		if order_dir="a" then
			AddDict smarty,"order_dir_ObsAnt","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_ObsAnt","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_ObsMod","a"
	if order_field="ObsMod" then
		if order_dir="a" then
			AddDict smarty,"order_dir_ObsMod","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_ObsMod","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_PrazoAnt","a"
	if order_field="PrazoAnt" then
		if order_dir="a" then
			AddDict smarty,"order_dir_PrazoAnt","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_PrazoAnt","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_PrazoMod","a"
	if order_field="PrazoMod" then
		if order_dir="a" then
			AddDict smarty,"order_dir_PrazoMod","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_PrazoMod","<img src=""images/" & img & ".gif"" border=0>"
	end if
	
	AddDict smarty,"order_dir_ID","a"
	if order_field="ID" then
		if order_dir="a" then
			AddDict smarty,"order_dir_ID","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_ID","<img src=""images/" & img & ".gif"" border=0>"
	end if
...
if request("mdelete[]").Count>0 then
	...
	for ind=1 to request("mdelete[]").Count
		AddDict keys,"ID",request("mdelete1[]").Item(request("mdelete[]").Item(ind))
...
'	write pagination
smarty.Add "pagination","<script language=""JavaScript"">WritePagination(" & mypage & "," & maxpages & ");function GotoPage(nPageNumber){window.location='_AudMoviment_list.asp?goto='+nPageNumber;}</script>"
...
'	CodMov - 
	value=""
				value = ProcessLargeText(GetData(rs,"CodMov", ""),"field=CodMov" & keylink,"",MODE_LIST)
			row.Add col & "CodMov_value",value
...
'	DataAlteracao - 
	value=""
				value = ProcessLargeText(GetData(rs,"DataAlteracao", ""),"field=DataAlteracao" & keylink,"",MODE_LIST)
			row.Add col & "DataAlteracao_value",value

'	DataAnt - 
	value=""
				value = ProcessLargeText(GetData(rs,"DataAnt", ""),"field=DataAnt" & keylink,"",MODE_LIST)
			row.Add col & "DataAnt_value",value

'	DataMod - 
	value=""
				value = ProcessLargeText(GetData(rs,"DataMod", ""),"field=DataMod" & keylink,"",MODE_LIST)
			row.Add col & "DataMod_value",value

'	OrigAnt - 
	value=""
				value = ProcessLargeText(GetData(rs,"OrigAnt", ""),"field=OrigAnt" & keylink,"",MODE_LIST)
			row.Add col & "OrigAnt_value",value

'	OrigMod - 
	value=""
				value = ProcessLargeText(GetData(rs,"OrigMod", ""),"field=OrigMod" & keylink,"",MODE_LIST)
			row.Add col & "OrigMod_value",value

'	DestAnt - 
	value=""
				value = ProcessLargeText(GetData(rs,"DestAnt", ""),"field=DestAnt" & keylink,"",MODE_LIST)
			row.Add col & "DestAnt_value",value

'	DestMod - 
	value=""
				value = ProcessLargeText(GetData(rs,"DestMod", ""),"field=DestMod" & keylink,"",MODE_LIST)
			row.Add col & "DestMod_value",value

'	ObsAnt - 
	value=""
				value = ProcessLargeText(GetData(rs,"ObsAnt", ""),"field=ObsAnt" & keylink,"",MODE_LIST)
			row.Add col & "ObsAnt_value",value

'	ObsMod - 
	value=""
				value = ProcessLargeText(GetData(rs,"ObsMod", ""),"field=ObsMod" & keylink,"",MODE_LIST)
			row.Add col & "ObsMod_value",value

'	PrazoAnt - 
	value=""
				value = ProcessLargeText(GetData(rs,"PrazoAnt", ""),"field=PrazoAnt" & keylink,"",MODE_LIST)
			row.Add col & "PrazoAnt_value",value

'	PrazoMod - 
	value=""
				value = ProcessLargeText(GetData(rs,"PrazoMod", ""),"field=PrazoMod" & keylink,"",MODE_LIST)
			row.Add col & "PrazoMod_value",value

'	ID - 
	value=""
				value = ProcessLargeText(GetData(rs,"ID", ""),"field=ID" & keylink,"",MODE_LIST)
			row.Add col & "ID_value",value
...
smarty_display("_AudMoviment_list.htm")
