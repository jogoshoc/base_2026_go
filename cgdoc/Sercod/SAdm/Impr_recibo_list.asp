...
if useAJAX then
	...
	includes = includes & "var MASTER_PREVIEW_TABLE = ""Impr_recibo_masterpreview.asp"";" & vbCrLf
end if
...
if request("mdelete[]").Count>0 then
	...
	for ind=1 to request("mdelete[]").Count
		...
		AddDict keys,"DtMovim",request("mdelete2[]").Item(request("mdelete[]").Item(ind))
...
'	write pagination
smarty.Add "pagination","<script language=""JavaScript"">WritePagination(" & mypage & "," & maxpages & ");function GotoPage(nPageNumber){window.location='Impr_recibo_list.asp?goto='+nPageNumber;}</script>"
...
'	key fields
			...
			row.Add col & "id2",my_htmlspecialchars(dbvalue(rs("DtMovim")))
...
'	edit page link
			...
			editlink=editlink & "&editid2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("DtMovim"))))
			row.Add col & "editlink",editlink

			copylink=""
			copylink=copylink & "copyid1=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("NrProtoc"))))
			copylink=copylink & "&copyid2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("DtMovim"))))
			row.Add col & "copylink",copylink
			keylink=""
			keylink=keylink & "&key1=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("NrProtoc"))))
			keylink=keylink & "&key2=" & my_htmlspecialchars(SafeURLEncode(dbvalue(rs("DtMovim"))))
...
smarty_display("Impr_recibo_list.htm")
