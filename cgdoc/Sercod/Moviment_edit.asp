...
rs.Open strSQL,dbConnection,1,2
if rs.EOF then
	response.redirect "Moviment_list.asp?a=return"
	response.end
end if
...
if useAJAX then
	includes=includes & "var AUTOCOMPLETE_TABLE='Moviment_autocomplete.asp';" & vbcrlf
	includes=includes & "var SUGGEST_TABLE='Moviment_searchsuggest.asp';" & vbcrlf
	includes=includes & "var SUGGEST_LOOKUP_TABLE='Moviment_lookupsuggest.asp';" & vbcrlf
end if
...
set readonlyfields = CreateObject("Scripting.Dictionary")
	...
	if readevalues then
  	...
  	readonlyfields.Add "PastaArquiv", my_htmlspecialchars(GetData(rs,"PastaArquiv", ""))
	if readevalues then
...
smarty_display("Moviment_edit.htm")
