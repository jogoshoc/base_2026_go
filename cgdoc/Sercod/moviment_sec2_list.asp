...
if useAJAX then
	includes = includes & "var SUGGEST_TABLE = ""moviment_sec2_searchsuggest.asp"";" & vbCrLf
	includes = includes & "var MASTER_PREVIEW_TABLE = ""moviment_sec2_masterpreview.asp"";" & vbCrLf
end if
...
smarty.Add "order_dir_Solucao","a"
...
if SESSION(strTableName & "_orderby")<> "" then
	...
	AddDict smarty,"order_dir_Solucao","a"
	if order_field="Solucao" then
		if order_dir="a" then
			AddDict smarty,"order_dir_Solucao","d"
			img="up"
		else
			img="down"
		end if
		AddDict smarty,"order_image_Solucao","<img src=""images/" & img & ".gif"" border=0>"
	end if
...
'	write pagination
smarty.Add "pagination","<script language=""JavaScript"">WritePagination(" & mypage & "," & maxpages & ");function GotoPage(nPageNumber){window.location='moviment_sec2_list.asp?goto='+nPageNumber;}</script>"
...
'	Solucao - 
	value=""
				value = ProcessLargeText(GetData(rs,"Solucao", ""),"field=Solucao" & keylink,"",MODE_LIST)
...
smarty_display("moviment_sec2_list.htm")
